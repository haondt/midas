...
{
    public class TransactionImportDataDto
    {
        ...
        {
            return JsonConvert.SerializeObject(sourceData, MidasConstants.ApiSerializerSettings);
        }

        public static List<string> DestringifySourceData(string sourceData)
        {
            return JsonConvert.DeserializeObject<List<string>>(sourceData, MidasConstants.ApiSerializerSettings) ?? throw new InvalidOperationException($"Unable to destringify source data {sourceData}");
        }
