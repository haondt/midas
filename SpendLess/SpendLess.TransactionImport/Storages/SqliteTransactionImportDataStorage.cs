...
namespace SpendLess.TransactionImport.Storages
{
    public class SqliteTransactionImportDataStorage : AbstractSqliteStorage, ITransactionImportDataStorage
    {
        ...
        public StorageOperation CreateAddManyOperation(List<TransactionImportData> datum)
        {
            return new CustomSqliteStorageOperation
            {
                Target = StorageKey<TransactionImportData>.Empty,
                Execute = (conn, trns) =>
                {
        ...
        public Task Add(TransactionImportData data) => AddMany([data]);

        public Task AddMany(IEnumerable<TransactionImportData> datum)
        {
        ...
        private void AddManyInternal(IEnumerable<TransactionImportData> datum, SqliteConnection conn, SqliteTransaction trns)
        {
            var columns = new List<(string Column, string Parameter, SqliteType Type, Func<TransactionImportData, object> Extractor)>
                {
            ...
            var setParametersList = columns
                .Select<(string Column, string Parameter, SqliteType Type, Func<TransactionImportData, object> Extractor), Action<TransactionImportData>>(c =>
                {
                    var parameter = command.Parameters.Add(c.Parameter, c.Type);
                    return d => parameter.Value = c.Extractor(d);
                }).ToList();
            void setParameters(TransactionImportData data)
            {
        ...
        public Task<List<List<TransactionImportData>>> GetMany(IEnumerable<long> transactionIds)
        {
            var result = WithConnection(conn =>
            {
                ...
                using var reader = command.ExecuteReader();
                var datum = new Dictionary<long, List<TransactionImportData>>();
                while (reader.Read())
                {
                    var data = new TransactionImportData
                    {
                        Transaction = reader.GetInt64(0),
                        Account = reader.GetString(1),
                        ConfigurationSlug = reader.GetString(2),
                        SourceData = TransactionImportData.DestringifySourceData(reader.GetString(3))
                    };
        ...
        public async Task<List<TransactionImportData>> Get(long transactionId)
        {
